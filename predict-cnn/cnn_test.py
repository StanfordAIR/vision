...
class Tests(tf.test.TestCase):
  ...
  def test_cnn(self):
    ...
    for i in range(3):
      ...
      self.assertEqual(predictions['probabilities'].shape, (NUM_CLASSES,))
      self.assertEqual(predictions['classes'].shape, ())

  def cnn_model_fn_helper(self, mode, multi_gpu=False):
    features, labels = dummy_input_fn()
    image_count = features.shape[0]
    spec = cnn.model_fn(features, labels, mode, {
        'data_format': 'channels_last',
        'multi_gpu': multi_gpu
    })
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      ...
      self.assertAllEqual(predictions['probabilities'].shape, (image_count, NUM_CLASSES))
  ...
  def test_cnn_model_fn_train_mode(self):
    self.cnn_model_fn_helper(tf.estimator.ModeKeys.TRAIN)

  def test_cnn_model_fn_train_mode_multi_gpu(self):
    self.cnn_model_fn_helper(tf.estimator.ModeKeys.TRAIN, multi_gpu=True)

  def test_cnn_model_fn_eval_mode(self):
    self.cnn_model_fn_helper(tf.estimator.ModeKeys.EVAL)

  def test_cnn_model_fn_predict_mode(self):
    self.cnn_model_fn_helper(tf.estimator.ModeKeys.PREDICT)
