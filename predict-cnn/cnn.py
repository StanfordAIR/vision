...
class Model(object):
  """Class that defines a graph to recognize digits in the  dataset."""
  ...
  def __init__(self, data_format):
    ...
    if data_format == 'channels_first':
      self._input_shape = [-1, 1, img_length, img_length]
    else:
      assert data_format == 'channels_last'
      self._input_shape = [-1, img_length, img_length, 1]
...
def main(unused_argv):
  ...
  def train_input_fn():
    ...
    # randomness, while smaller sizes use less memory.  is a small
  ...
  print("test 1")
  _classifier.train(input_fn=train_input_fn, hooks=[logging_hook])
  print("test 2")
  ...
  if FLAGS.export_dir is not None:
    image = tf.placeholder(tf.float32, [None, img_length, img_length])
    input_fn = tf.estimator.export.build_raw_serving_input_receiver_fn({
        'image': image,
    })
    _classifier.export_savedmodel(FLAGS.export_dir, input_fn)
    print("SavedModel saved to: "+FLAGS.export_dir)
